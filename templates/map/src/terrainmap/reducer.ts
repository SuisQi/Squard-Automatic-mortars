...
const defaultTerainmap = (): Terrainmap => ({
...
const transition = (state: Terrainmap, payload: keyof (typeof maps)) => {
  ...
  texture.source = mapData.terrainmap_image_src;
  texture.image.src = mapData.terrainmap_image_src;
...
export const terrainmap = (state: any, action: any) => {
  if (state === undefined){
      return defaultTerainmap();
  }
